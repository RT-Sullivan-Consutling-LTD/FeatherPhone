...
#define ROTARY_DIAL_TIMEOUT 4000 //If no numbers are dialed for this period, newline is sent
...
void setup() {
  ...
  // show debug
  Serial.begin(115200);
  Serial.println(F("Feather Phone - RT Sullivan Consulting, Ltd"));
...
void loop() {
  ...
  if(HOOKEVT){ // only when the hook event changes
    ...
    if(digitalRead(HOOK)==OFFHOOK){
      ...
      // on hook. 
      fona.playToolkitTone(1, 10); // Stop any playing tones
      fona.hangUp();
     // fona.setVolume(90);
    }
  } // end HOOKEVT
